...
def index(request):
    ...
    # projects_list_url = reverse('projects_list')
    # page_main_quality_control = reverse('page_main_quality_control')
    # html = f'''
    # <h1>Страница приложения tasks</h1><a href='{projects_list_url}'>Список проектов</a>
    # <br>
    # <a href='{page_main_quality_control}'>Перейти на главную страницу системы контроля качества</a>
    # '''
...
def projects_list(request):
    ...
    return render(request, 'tasks/projects_list.html', {
...
def update_task(request, project_id, task_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('task_detail', project_id=project_id, task_id=task.id)
    else:
...
class TaskDeleteView(DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy('project_detail', kwargs={'project_id': self.object.project.id})
